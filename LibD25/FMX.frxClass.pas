...
{ Report Objects }
  ...
  public
    ...
    function WrapText(WrapWords: Boolean; var aLineHeight: Single; aBitmap: TBitmap = nil): WideString;
    procedure Draw(Canvas: TCanvas; ScaleX, ScaleY, OffsetX, OffsetY: Extended); override;
    procedure BeforePrint; override;
    procedure GetData; override;
    procedure AfterPrint; override;
...
{$ENDIF}
  ...
  frxCanvasClass: TfrxCanvasClass;
const
...
begin
  ...
  begin
    ...
    if IsAncestor and not((Self is TfrxReport) and (Name = '')) then
...
function TfrxCustomMemoView.WrapText(WrapWords: Boolean; var aLineHeight: Single; aBitmap: TBitmap): WideString;
...
var
    ...
    s1, s2, s3: String;
    nLen, nIndex, idx, idy, LWordIndex: Integer;
    TextCanvas: TCanvas;
    FBmpCanvas: TBitmap;
    HasWords: Boolean;
    aText: String;
    ...
    begin
      ...
      begin
        if TextCanvas.TextWidth(Copy(sText, 1, RemaingLen + 1)) + aSpeceWidth < Trunc(FTextRect.Width - AddTextWidth) then
          Inc(Len)
        else
          break;
      end;
      if (CurWidth + aSpeceWidth> Trunc(FTextRect.Width - AddTextWidth)) then
      begin
        RemaingLen := RemaingLen - Len;
        Result := Copy(Result, 1, RemaingLen);
      end
      else if (CurWidth + aSpeceWidth < Trunc(FTextRect.Width - AddTextWidth)) then
      begin
        RemaingLen := RemaingLen + Len;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            if Line.IsLast then
              break;
            nLen := paragraph.Lines[idy + 1].OriginalCharIndex - nIndex;
          ...
          begin
            ...
            if (s1[nIndex] = #10) then
              Dec(nIndex);
            nLen := paragraph.Lines[0].OriginalCharIndex - nIndex;
            //if idx = 0 then
{$IFNDEF MACOS}
            Dec(nLen);
{$ENDIF}
...
//      aSpeceWidth := Round(FBmpCanvas.Canvas.TextWidth('     ') / 5 );
//{$ELSE}
//      aSpeceWidth := 0;
//{$ENDIF}
//{$ELSE}
...
//      aSpeceWidth := 0;
//{$ELSE}
//      aSpeceWidth := FBmpCanvas.Canvas.TextWidth(' ');
//{$ENDIF}
//{$ENDIF}
        ...
        begin
          ...
          begin
            ...
            if (nIndex = Length(s1)) then
              s2 := s2 + s1[nIndex];
            if (TextCanvas.TextWidth(s2) + aSpeceWidth > Trunc(FTextRect.Width - AddTextWidth)) then
            begin
              if not HasWords then
              begin
                s3 := SplitString(s2);
                Result := Result + s3 + sLineBreak;
                nIndex := LWordIndex + Length(s3);
                LWordIndex := LWordIndex + Length(s3);
              end
              else
              begin
                Result := Result + Copy(s2, 1, Length(s2) - (nIndex - LWordIndex))  + sLineBreak;
                Inc(LWordIndex);
                nIndex := LWordIndex;

                //LWordIndex + Length(s3);
//                LWordIndex := LWordIndex + Length(s3);
              end;
              HasWords := False;
              s2 := '';
        ...
        if s2 <> '' then
          Result := Result + s2 + sLineBreak;
      end;
      aLineHeight := Font.GetHeight(FBmpCanvas.Canvas) + {$IFDEF MACOS}1{$ELSE}1.5{$ENDIF};
    end;
  finally
    TextCanvas.EndScene;
