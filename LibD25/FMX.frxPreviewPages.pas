...
uses
  ...
  FMX.frxClass, FMX.frxXML, FMX.frxPictureCache, System.Diagnostics, System.TimeSpan, FMX.frxXMLSerializer, FMX.frxFMX
{$IFDEF DELPHI19}
  , FMX.Graphics
{$ENDIF}
{$IFDEF DELPHI19}
...
type
  ...
  private
    ...
    FPageBitmapCache: TStringList;  { last 20 TfrxPreviewPage bitmaps }
    ...
    FLastScale: Extended;
    FSerializer: TfrxXMLSerializer;
    FFastCanvas: TCanvas;
    FFastPrintCanvas: TfrxFastCanvasLayer;
    procedure AfterLoad;
    procedure BeforeSave;
    procedure ClearPageCache;
    procedure ClearPageBitmapCache;
    ...
    procedure DrawPageInternal(Index: Integer; Canvas: TCanvas;
      ScaleX, ScaleY, OffsetX, OffsetY: Extended; WorkArea: TRectF);
  protected
    function GetCount: Integer; override;
  ...
  public
    ...
    procedure DrawPage(Index: Integer; Canvas: TCanvas; ScaleX, ScaleY,
      OffsetX, OffsetY: Extended; WorkArea: TRectF); override;
...
uses
  FMX.frxPreview, FMX.frxPrinter,
  FMX.frxUtils, FMX.frxDMPClass, FMX.frxRes, FMX.frxPrintDialog, FMX.Dialogs,
...
begin
  ...
  FPageCache := TfrxStringList.Create;
  FPageBitmapCache := TStringList.Create;
  FPictureCache := TfrxPictureCache.Create;
  FSerializer := TfrxXMLSerializer.Create(nil);
  FLastY := 0;
  FFastPrintCanvas := nil;
end;
...
begin
  ...
  FXMLDoc.Free;
  FSerializer.Free;
  if Assigned(FFastPrintCanvas) then
    FFastPrintCanvas.Free;
  inherited;
end;
...
procedure TfrxPreviewPages.ClearPageBitmapCache;
begin
  while FPageBitmapCache.Count > 0 do
  begin
    TBitmap(FPageBitmapCache.Objects[0]).Free;
    FPageBitmapCache.Delete(0);
  end;
end;
...
begin
  ...
  begin
    Item := AnchorRoot[i];
    if frxCompareText(Item.Prop['text'], Text) = 0 then
    begin
      Result := Item;
      Exit;
    end;
...
{$IFDEF FRVIEWER}
{$IFDEF MSWINDOWS}
      RepStream := TStringStream.Create;
      try
        RepStream.LoadFromFile(FileName);
        if Pos('PixelsPerInch' ,RepStream.DataString, 1)= 0 then
        begin
           ShellExecute(0, 'open', PChar(ExtractFilePath(ParamStr(0)) + 'FastReportVCL_Viewer.exe'), PChar(FileName), '', 0);
...
var
  ...
  begin
    ...
    begin
      ...
      if c0 = nil then
        c := FSerializer.ReadComponentStr(Owner, Item[i].Name + ' ' + Item[i].Text, True)
      else
      begin
        c := FSerializer.ReadComponentStr(Owner,
          THackComponent(c0).FBaseName + ' ' + Item[i].Text, True);
        c.Name := c0.Name;
        if (c is TfrxPictureView) and (TfrxPictureView(c).Picture.IsEmpty) then
...
begin
  ...
  FSerializer.OldFormat := FXMLDoc.OldVersion;
  ...
  begin
    ...
    FSerializer.ReadRootComponent(Result, xi);
  end
...
procedure TfrxPreviewPages.DrawPageInternal(Index: Integer; Canvas: TCanvas;
  ScaleX, ScaleY, OffsetX, OffsetY: Extended; WorkArea: TRectF);
var
  i, pCount: Integer;
  Page: TfrxReportPage;
...
begin
  ...
  IsPrinting := THackCanvas(Canvas).FPrinter <> nil;
  ...
  begin
    ...
    if (c is TfrxView) then
      ...
      begin
        ...
        begin
          c.IsPrinting := IsPrinting;
          THackMemoView(c).FTotalPages := pCount;
          THackMemoView(c).FCopyNo := FCopyNo;
          THackMemoView(c).FPrintScale := FPrintScale;
        end;
        { draw the object }
        if WorkArea.Width = 1 then
          WorkArea.Right := WorkArea.Right + 1

        else if WorkArea.Height = 1 then
          WorkArea.Bottom := WorkArea.Bottom + 1;


        TfrxView(c).GetVisibleRect(ObjectRect);

        ObjectRect.Left := ObjectRect.Left * ScaleX + OffsetX;
        ObjectRect.Top := ObjectRect.Top * ScaleY + OffsetY;

        if ObjectRect.Left < WorkArea.Left then
          ObjectRect.Left := WorkArea.Left;
        if ObjectRect.Top < WorkArea.Top then
          ObjectRect.Top := WorkArea.Top;

        ObjectRect.Right := ObjectRect.Right * ScaleX + OffsetX;
        ObjectRect.Bottom := ObjectRect.Bottom * ScaleY + OffsetY;
        if ObjectRect.Right > WorkArea.Right then
          ObjectRect.Right := WorkArea.Right - 1;
        if ObjectRect.Bottom > WorkArea.Bottom then
          ObjectRect.Bottom := WorkArea.Bottom - 1;

        if WorkArea.Contains(PointF(ObjectRect.Left + 1, ObjectRect.Top + 1)) and
          WorkArea.Contains(PointF(ObjectRect.Right - 1, ObjectRect.Top + 1)) and
          WorkArea.Contains(PointF(ObjectRect.Left + 1, ObjectRect.Bottom - 1)) and
          WorkArea.Contains(PointF(ObjectRect.Right - 1, ObjectRect.Bottom - 1))
        { zero rect means we want to draw everything, print or export }
          or ((WorkArea.Top = 0) and (WorkArea.Left = 0) and
          (WorkArea.Right = 0) and (WorkArea.Bottom = 0)) then
        begin
...
begin
  ...
  IsPrinting := THackCanvas(Canvas).FPrinter <> nil;
{$IFDEF DELPHI25}
  if (Report <> nil) and Assigned(FFastPrintCanvas) then
  begin
    FFastPrintCanvas.Canvas := Canvas;
    FFastCanvas := FFastPrintCanvas;
  end;
{$ENDIF}
  if IsPrinting or Engine.Running then
  begin
    DrawPageInternal(Index, Canvas, ScaleX, ScaleY, OffsetX, OffsetY, WorkArea);
    Exit;
  end;
  if (Report <> nil) and (Report.Preview <> nil) then
    FFastCanvas := Report.Preview.GetCanvas;

  if FLastScale <> ScaleX then
  begin
    ClearPageBitmapCache;
    FLastScale := ScaleX;
  end;
  i := FPageBitmapCache.IndexOf(IntToStr(Index));
  if i <> -1 then
  begin
    Bitmap := TBitmap(FPageBitmapCache.Objects[i]);
    FPageBitmapCache.Exchange(i, 0);
  end
  else
  begin
    Page := GetPage(Index);
    if Page = nil then Exit;
    if Report.PreviewOptions.PagesInPictureCache and
      (Round(Page.Width * ScaleX) < Report.PreviewOptions.MaxPagePictureWidth)
      and (Round(Page.Height * ScaleX) <
      Report.PreviewOptions.MaxPagePictureHeight) then
    begin
      Bitmap := TBitmap.Create(Round(Page.Width * ScaleX),
        Round(Page.Height * ScaleY));
      Bitmap.Canvas.BeginScene;
      try
        Bitmap.Canvas.Clear(claWhite);
        DrawPageInternal(Index, Bitmap.Canvas, ScaleX, ScaleY, 0, 0,
          RectF(0, 0, 0, 0));
      finally
        Bitmap.Canvas.EndScene;
      end;
      { add this page to the pagecache }
      FPageBitmapCache.InsertObject(0, IntToStr(Index), Bitmap);
      i := FPageBitmapCache.Count;

      { remove the least used item from the pagecache }
      if (i > 1) and (i > Report.PreviewOptions.PagesInCache) then
      begin
        TBitmap(FPageBitmapCache.Objects[i - 1]).Free;
        FPageBitmapCache.Delete(i - 1);
      end;
    end;
  end;
  ...
  if Bitmap <> nil then
    ...
    RectF(OffsetX, OffsetY, OffsetX + Bitmap.Width, OffsetY + Bitmap.Height), 1)
  else
    DrawPageInternal(Index, Canvas, ScaleX, ScaleY, OffsetX, OffsetY, WorkArea);
end;
...
(*          SavedDCState := SaveDC(Printer.Canvas.Handle);
          ...
          OutRgn :=  CreateRectRgn(Round(SplitAddX + Printer.LeftMargin * Printer.DPI.X / 25.4),
                        ...
                        Round(Page.PaperWidth * fr01cm * Printer.DPI.X / frxDefPPI - Printer.RightMargin * Printer.DPI.X / 25.4) - 1,
                        Round(Page.PaperHeight * fr01cm * Printer.DPI.Y / frxDefPPI - Printer.BottomMargin * Printer.DPI.Y / 25.4) - 1);
          SelectClipRgn(Printer.Canvas.Handle, OutRgn);
          SetMetaRgn(Printer.Canvas.Handle);
          try
            Printer.BeginPage;
            DrawPage(Index, Printer.Canvas, Printer.DPI.X / frxDefPPI, Printer.DPI.Y / frxDefPPI,
              offsX, offsY);
...
//        offsY := -printedY;
    ...
    begin
      ...
      DrawPage(Index, Printer.Canvas, Printer.DPI.X / frxDefPPI * ZoomX, Printer.DPI.Y / frxDefPPI * ZoomY,
        -Printer.LeftMargin * Printer.DPI.X / 25.4,
        -Printer.TopMargin * Printer.DPI.Y / 25.4, RectF(0, 0, 0, 0));
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            DrawPage(GetPageIndex(Index), Printer.Canvas, Printer.DPI.X / frxDefPPI, Printer.DPI.Y / frxDefPPI,
              offsX, offsY, RectF(0, 0, 0, 0));
...
begin
  ...
  if Report.PrintOptions.ShowDialog then
    ...
    begin
      ...
      if PrintRange = TPrintRange.prPageNums then
        Report.PrintOptions.PageNumbers := IntToStr(FromPage) + '-' + IntToStr(ToPage);
      Free;
    end;
    if Report.PreviewForm is TfrxForm then
      TfrxForm(Report.PreviewForm).PeekLastModalResult;
...
// todo
  if Report.PrintOptions.ShowDialog then
    with TfrxPrintDialog.Create(nil) do
    begin
      AReport := Report;
...
{$IFDEF MSWINDOWS}
    if Assigned(frxCanvasClass) then
      FFastPrintCanvas := frxCanvasClass.Create;
{$ENDIF}
  try
  ...
  var
    ...
    begin
      ...
      if c is TfrxView then
        begin
          if Assigned(FFastPrintCanvas) then
            TfrxView(c).SetFastCanvas(FFastPrintCanvas)
          else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        with v do
          aRect := RectF(Round(AbsLeft * Scale), Round(AbsTop * Scale),
                        Round((AbsLeft + Width) * Scale),
                        Round((AbsTop + Height) * Scale));
          OffsetRect(aRect, Round(OffsetX), Round(OffsetY));
          Report.Preview.ShowHint(aRect, GetShortHint(v.Hint));
          v.HintIsActive := True;
          //GetShortHint(v.Hint);
...
{$IFDEF MSWINDOWS}
            ShellExecute(0, 'open', PChar(v.URL), '', '', 1);
{$ENDIF}
{$IFDEF MACOS}
            _system(PAnsiChar('open ' + AnsiString(v.URL)));
{$ENDIF}
        Flag := False;
        if DBClick then
        ...
        begin
          ...
          Report.Preview.ClearBackBuffer;
          Report.Preview.Repaint;
        end;
      end
    ...
    begin
      if (TfrxView(c).ShowHint) and (TfrxView(c).HintIsActive) and (Report.Preview <> nil) and (Report.Preview.UseReportHints) then
      begin
        Report.Preview.HideHint;
        TfrxView(c).HintIsActive := False;
      end;
    end;
  end;
