...
uses
  ...
  FMX.Forms,
  FMX.frxClass, FMX.frxAggregate, FMX.frxXML, FMX.frxDMPClass;
...
type
  ...
  public
    ...
    function HeaderHeight: Double; override;
...
uses
  FMX.frxUtils, FMX.frxPreviewPages, FMX.frxRes, FMX.frxFMX;
  ...
  begin
    ...
    for i := 0 to Report.PagesCount - 1 do
      ...
      begin
        ...
        if p.DialogForm.ChildrenCount <> 0 then
        begin
...
begin
  SortBands := TfrxStringList.Create;
...
function TfrxEngine.HeaderHeight: Double;
var
  Band: TfrxBand;
begin
  Result := 0;
...
function TfrxEngine.FooterHeight: Double;
var
  Band: TfrxBand;
begin
  Result := 0;
...
function TfrxEngine.FreeSpace: Double;
begin
  if FPage.EndlessHeight then
    Result := 1e+6
  else
...
begin
  ...
  begin
    ...
    begin
      TfrxStretcheable(c).SetFastCanvas(TfrxFastCanvaslayer(FFastCanvas));
      h := TfrxStretcheable(c).CalcHeight;
      TfrxStretcheable(c).SetFastCanvas(nil);
      if h > c.Height then
      begin
...
{$ENDIF}
    ...
    begin
      ...
      begin
        ...
        StrView.SetFastCanvas(TfrxFastCanvaslayer(FFastCanvas));
...
function TfrxEngine.GetPageHeight: Double;
begin
