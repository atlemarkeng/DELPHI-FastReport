...
uses
  ...
  System.Variants, FMX.Edit, FMX.ListBox, FMX.frxBaseModalForm
{$IFDEF DELPHI18}
  ,FMX.StdCtrls
{$ENDIF}
{$IFDEF DELPHI19}
...
type
  TfrxHTMLExportDialog = class(TfrxForm)
...
uses FMX.frxUtils, {FMX.frxFileUtils,} FMX.frxUnicodeUtils, FMX.frxRes, FMX.frxrcExports, System.Math, FMX.frxGraphicUtils, FMX.frxFMX
 {$IFDEF MSWINDOWS}
 , Winapi.ShellAPI
 {$ENDIF};
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        sb := '';
        si := '';
        if TFontStyle.fsBold in EStyle.Font.Style then
          sb := ' font-weight: bold;'
        else
          sb := '';
        if TFontStyle.fsItalic in EStyle.Font.Style then
          si := ' font-style: italic;'
        else
          si := ' font-style: normal;';

        if TFontStyle.fsUnderline in EStyle.Font.Style then
          su := ' text-decoration: underline';
        if TFontStyle.fsStrikeout in EStyle.Font.Style then
        begin
          if su = '' then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Files.Add(s);
            s := ReverseSlash(GetPicsFolderRel + 'img' + IntToStr(FPicturesCount) + '.' + GetPicFileExtension(PictureType));
...
begin
  ...
  begin
    ...
    begin
      ...
      StylesCB.IsChecked := FExportStyles;
      PicsSameCB.IsChecked := FPicsInSameFolder;
      ...
      OpenAfterCB.IsChecked := FOpenAfterExport;
      FixWidthCB.IsChecked := FFixedWidth;
      NavigatorCB.IsChecked := FNavigator;
      MultipageCB.IsChecked := FMultipage;
      ...
      begin
        ...
        FExportStyles := StylesCB.IsChecked;
        FPicsInSameFolder := PicsSameCB.IsChecked;
        ...
        FOpenAfterExport := OpenAfterCB.IsChecked;
        FFixedWidth := FixWidthCB.IsChecked;
        FMultipage := MultipageCB.IsChecked;
        FNavigator := NavigatorCB.IsChecked;
...
{$IFDEF MSWINDOWS}
      ShellExecute(0, 'open', PChar(GetPicsFolder + '1.html'), '', '', 5)
{$ENDIF}
{$IFDEF MACOS}
      ShellExecute(GetPicsFolder + '1.html')
{$ENDIF}
    else
{$IFDEF MSWINDOWS}
...
begin
  if FPicsInSameFolder then
    ...
    Result := ExtractFilePath(FileName) + ChangeFileExt(ExtractFileName(frxUnixPath2WinPath(FileName)),'.files') + PathDelim
  else if FNavigator then
    Result := ''
  else
    Result := ChangeFileExt(ExtractFileName(frxUnixPath2WinPath(FileName)),'.files') + PathDelim
end;
...
begin
  ...
  OpenAfterCB.Text := frxGet(8201);
  StylesCB.Text := frxGet(8202);
  PicturesL.Text := frxGet(8203);
  PicsSameCB.Text := frxGet(8204);
  FixWidthCB.Text := frxGet(8205);
  NavigatorCB.Text := frxGet(8206);
  MultipageCB.Text := frxGet(8207);
  BackgrCB.Text := frxGet(8209);
