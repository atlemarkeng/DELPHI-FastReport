  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //pdf.Write(pdfFont.FontData.Memory, pdfFont.FontDataSize);
        fontstream.CopyFrom(pdfFont.FontData, pdfFont.FontData.Size);
    end;
    WriteLn(pdf, '');
    WriteLn(pdf, 'endstream');
...
{$ENDIF}
    try
      txt.Add(Text);
      FLineWidth := TextRenderer.MeasureTextWidth(txt.Text);
...
begin
  ...
  begin
    ...
    try
      ...
      textObj.Font.AssignToCanvas(BmpCanvas.Canvas);
      ...
      BmpCanvas.Canvas.Clear(claNull);
      TextRenderer := TAdvancedTextRenderer.Create(BmpCanvas.Canvas, textObj.Memo.Text, textObj.Font, FTextRect, textObj.HAlign, textObj.VAlign, -textObj.Font.Height, textObj.Rotation, textObj.WordWrap, False, textObj.AllowHTMLTags, True);
      BmpCanvas.Canvas.EndScene;
      FLineHeight := TextRenderer.LineHeight;
...
{$ENDIF}
      Lines.Text := textObj.WrapText(textObj.WordBreak);
...
{$IFDEF Delphi12}
        Write(OutStream, TfrxPDFFont(FFonts[FontIndex]).Name +
          AnsiString(' ' + IntToStr(Round(textObj.Font.Size)) + ' Tf'#13#10));
{$ELSE}
        Write(OutStream, TfrxPDFFont(FFonts[FontIndex]).Name +
  ...
  begin
    ...
    try
      ...
      try
        ...
        len := TrueTypeTables.FamilyItem[SourceFont.Family].GetGlyphIndices(text, tempGlyphs, tempWidths, rtl, FComposedList);
      end;
...
begin
  ...
  fnt := CreateFont(0, 0, 0, 0, fBold, fItalic, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 0, PWideChar(SourceFont.Family));
  try
...
begin
{$IFDEF Delphi12}
  Result := AnsiString(SourceFont.Family);
...
{$ENDIF}
  if frxPDFFontsNeedStyleSimulation.IndexOf(SourceFont.Family) = -1 then
  begin
    s := '';
