...
//            if (collate) then self.ListOfUsedGlyphs[key] := idx;
            ...
            begin
              ...
              begin
                ...
                GlyphWidth := ((self.horizontal_metrix_table.Item[idx].advanceWidth * $3e8) div self.font_header.unitsPerEm) - 24;
...
//        FTextMetric.otmPanoseNumber.ArmStyle := self.windows_metrix.Panose[6];
        ...
        FTextMetric.otmpFamilyName := PAnsiChar(self.name_table.Item[NameID.FamilyName]);
            ...
            begin
                case ( TablesID(table.tag) ) of
                    TablesID.HorizontalHeader:
                        begin parsed_table := HorizontalHeaderClass.Create(table); end;
                    TablesID.FontHeader:
                        begin parsed_table := FontHeaderClass.Create(table); end;
                    TablesID.Name:
                        begin parsed_table := NameTableClass.Create(table); end;
                    TablesID.OS2Table:
                        begin parsed_table := OS2WindowsMetricsClass.Create(table); end;
                    TablesID.GlyphSubstitution:
                        begin parsed_table := GlyphSubstitutionClass.Create(table); end;
                    TablesID.IndexToLocation:
                        begin parsed_table := IndexToLocationClass.Create(table); end;
                    TablesID.Glyph:
                        begin parsed_table := GlyphTableClass.Create(table); end;
                    TablesID.KerningTable:
                        begin parsed_table := KerningTableClass.Create(table); end;
                    TablesID.CMAP:
                        begin parsed_table := CmapTableClass.Create(table); end;
                    TablesID.Postscript:
                        begin parsed_table := PostScriptClass.Create(table); end;
                    TablesID.HorizontalMetrix:
                        begin parsed_table := HorizontalMetrixClass.Create(table); end;
                    TablesID.PreProgram:
                        begin parsed_table := PreProgramClass.Create(table); end;
                    TablesID.MaximumProfile:
                        begin parsed_table := MaximumProfileClass.Create(table); end;
                    else
