...
uses
  System.SysUtils, System.Classes, FMX.Controls, System.UITypes, System.UIConsts, System.Types, FMX.Types, FMX.Forms,
...
type
  ...
  public
    constructor Create;
    procedure Add(AObj: TObject; ACoord: Double);
  ...
  public
    ...
    function RowHeaderWidth: Double;
    function ColumnHeaderHeight: Double;
  ...
  published
    property AddHeight: Double read FAddHeight write FAddHeight;
    property AddWidth: Double read FAddWidth write FAddWidth;
    ...
    property NextCrossGap: Double read FNextCrossGap write FNextCrossGap;
...
{$IFNDEF NO_EDITORS}
  FMX.frxCrossEditor,
{$ENDIF}
  FMX.frxCrossRTTI, FMX.frxDsgnIntf, FMX.frxXML, FMX.frxUtils, FMX.frxXMLSerializer, FMX.frxRes,
  FMX.frxDMPClass, FMX.frxVariables, FMX.frxUnicodeUtils;
    ...
    for i := 0 to FIndexesCount - 1 do
      ...
      begin
        if (VarType(Indexes[i]) = varString) or (VarType(Indexes[i]) = varOleStr)
          or (VarType(Indexes[i]) = varUString) then
...
function TfrxCrossHeader.GetHeight: Double;
var
  Items: TList;
begin
  Items := TerminalItems;
...
function TfrxCrossHeader.GetWidth: Double;
var
  Items: TList;
begin
  Items := TerminalItems;
...
procedure TfrxGridLines.Add(AObj: TObject; ACoord: Double);
var
  i: Integer;
...
function TfrxCustomCrossView.ColumnHeaderHeight: Double;
begin
  Result := ColumnHeader.Height;
end;

function TfrxCustomCrossView.RowHeaderWidth: Double;
begin
...
begin
  ...
  begin
    ...
    if i = 0 then
      ...
      m.Text := 'Total';
    m.Font.Style := [TFontStyle.fsBold];
    m.HAlign := haCenter;
    m.VAlign := vaCenter;
    m.Frame.Typ := [ftLeft, ftRight, ftTop, ftBottom];
  end;
...
begin
  ...
  begin
    ...
    if Value = Null then
      Cell.Value := v
    else if (TVarData(Value).VType = varString) or (TVarData(Value).VType = varOleStr) or (TVarData(Value).VType = varUString) then
    begin
    ...
    for i := 0 to FGridX.Count - 1 do
      ...
      begin
        ...
        if (Abs(c.AbsLeft + c.Width - X / FScaleX) < 2) and
          (Y / FScaleY >= c.AbsTop) and (Y / FScaleY <= c.AbsTop + c.Height) then
        begin
          TControl(Sender).Cursor := crHSplit;
          break;
        end;
      end;
    ...
    for i := 0 to FGridY.Count - 1 do
      ...
      begin
        ...
        if (Abs(c.AbsTop + c.Height - Y / FScaleY) < 2) and
          (X / FScaleX >= c.AbsLeft) and (X / FScaleX <= c.AbsLeft + c.Width) then
        begin
          TControl(Sender).Cursor := crVSplit;
          break;
        end;
      end;
  end
...
function TfrxCustomCrossView.DrawCross(Canvas: TCanvas; ScaleX, ScaleY,
  ...
  procedure DrawLine2(x, y, dx, dy: Extended);
  begin
    Canvas.DrawLine(PointF(Round(x * ScaleX), Round(y * ScaleY)), PointF(Round((x + dx) * ScaleX), Round((y + dy) * ScaleY)), 1);
  end;
  ...
  begin
    if (Canvas <> nil) and (c.OnBeforePrint <> '') then
      ...
      begin
        ...
        DrawLine2(AbsLeft + 2, AbsTop + 1, 0, 7);
        DrawLine2(AbsLeft + 3, AbsTop + 2, 0, 5);
        DrawLine2(AbsLeft + 4, AbsTop + 3, 0, 3);
        DrawLine2(AbsLeft + 5, AbsTop + 4, 0, 1);
      end;
  end;
  ...
  begin
    ...
    begin
      ...
      if THackMemoView(Memo).HideZeros and (TVarData(THackMemoView(Memo).Value).VType <> varString)
        and (TVarData(THackMemoView(Memo).Value).VType <> varUString) and
...
initialization
  ...
  GroupDescendentsWith(TfrxCrossObject, TFmxObject);
  frxObjects.RegisterObject1(TfrxCrossView, nil, '', '', 0, 142, [ctReport, ctDMP]);
  frxObjects.RegisterObject1(TfrxDBCrossView, nil, '', '', 0, 149, [ctReport, ctDMP]);
