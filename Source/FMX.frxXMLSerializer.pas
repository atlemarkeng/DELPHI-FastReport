...
uses 
  System.UITypes, FMX.Types, FMX.frxUtils, FMX.frxRes, FMX.frxUnicodeUtils, FMX.frxFMX;
...
begin
  ...
  FOldFormat := False;
  FCachedClasses := TfrxStringList.Create;
  FCachedClasses.Sorted := True;
end;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            else if Obj1 is TfrxWideStrings then
            begin
              TfrxWideStrings(Obj1).Text := frxXMLToStr(Value);
              continue;
            end
        ...
        if (p <> nil) and (p.SetProc <> nil) then
          case p.PropType^.Kind of
            ...
            tkInt64:
              SetOrdProp(Obj1, p, StrToInt64(String(Value)));
  ...
  begin
    ...
    if not IsDefault or FSerializeDefaultValues then
      if PropList[i].PropType^.Kind = tkInt64 then
        s := IntToStr(Int64(Value))
      else if PropList[i].PropType^.Kind = tkEnumeration then
      begin
...
// commented out due to bug with tfrxmemoview.linespacing=0
  ...
  begin
    ...
    begin
      ...
      else if FObj is TfrxWideStrings then
      begin
    ...
    begin
      ...
      if IsStoredProp(Obj, PropList[i]) then
        case PropList[i].PropType^.Kind of
          tkInt64, tkInteger, tkSet, tkChar, tkWChar, tkEnumeration:
            DoOrdProp;
  ...
  begin
    ...
    if not IsAncestor then
      try
        ...
        i := FCachedClasses.IndexOf(Item.Name);
        if i = -1 then
          i := FCachedClasses.AddObject(Item.Name, TObject(FindClass(Item.Name)));
//      Result := TfrxComponent(FindClass(s).NewInstance);
        aClass := TPersistentClass(FCachedClasses.Objects[i]);
//        aClass := FindClass(Item.Name);
      except
    ...
    begin
      ...
      begin
        c := TfrxComponent(aClass.NewInstance);
        c.Create(Owner);
      end;
    end
    else
...
begin
  ...
  begin
    ...
    n := FCachedClasses.IndexOf(s);
    if n = -1 then
      n := FCachedClasses.AddObject(s, TObject(FindClass(s)));
//      Result := TfrxComponent(FindClass(s).NewInstance);
    Result := TfrxComponent(TPersistentClass(FCachedClasses.Objects[n]).NewInstance);
    Result.Create(Root);
