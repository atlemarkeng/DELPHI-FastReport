...
uses
  System.Classes, FMX.frxClass, FMX.frxCustomDB, Data.DB, Data.DBCommonTypes,
  Data.SqlExpr, Datasnap.Provider, Datasnap.DBClient, System.Variants, FMX.Types
{$IFDEF QBUILDER}
, FMX.fqbClass
{$ENDIF};
...
initialization
  ...
  GroupDescendentsWith(TfrxDBXDataBase, TFmxObject);
  GroupDescendentsWith(TfrxDBXTable, TFmxObject);
  GroupDescendentsWith(TfrxDBXQuery, TFmxObject);
  frxObjects.RegisterObject1(TfrxDBXDataBase, nil, '', {$IFDEF DB_CAT}'DATABASES'{$ELSE}''{$ENDIF}, 0, 246);
  frxObjects.RegisterObject1(TfrxDBXTable, nil, '', {$IFDEF DB_CAT}'TABLES'{$ELSE}''{$ENDIF}, 0, 247);
  frxObjects.RegisterObject1(TfrxDBXQuery, nil, '', {$IFDEF DB_CAT}'QUERIES'{$ELSE}''{$ENDIF}, 0, 248);
