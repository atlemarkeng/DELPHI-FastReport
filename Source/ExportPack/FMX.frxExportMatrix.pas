...
uses
  System.SysUtils, System.Classes, FMX.Types, System.UITypes, System.Types, FMX.frxClass, FMX.frxPreviewPages,
  FMX.frxProgress, {Printers,} FMX.frxUtils, FMX.frxUnicodeUtils, FMX.frxfmx, System.WideStrings, System.UIConsts
{$IFDEF DELPHI19}
  , FMX.Graphics
{$ENDIF};
...
uses fmx.frxres, FMX.frxrcExports;
...
begin
  ...
  begin
    ...
    if (FDeleteHTMLTags and TfrxCustomMemoView(Obj).AllowHTMLTags) or FWrap then
      FObj.Memo.Text := TfrxCustomMemoView(Obj).WrapText(True, aLineHeight)
    else
  ...
  begin
    ...
    begin
      ...
      begin
        FObj.Image := TBitmap.Create(Round(dx) + 1, Round(dy) + 1);
        FObj.Image.Canvas.BeginScene();
        FObj.Image.Canvas.Clear(claWhite);
        if (Obj is TfrxCustomLineView) and (OldFrameWidth > 0) then
          Obj.Frame.Width := OldFrameWidth;
        TfrxView(Obj).Draw(FObj.Image.Canvas, 1, 1, -DrawPosX, -DrawPosY);
        FObj.Image.Canvas.EndScene();
        if OldFrameWidth > 0 then
          Obj.Frame.Width := OldFrameWidth;
      end;
    end
...
begin
  Result := (Obj is TfrxCustomMemoView) and
            ((Obj.BrushStyle in [TBrushKind.bkSolid, TBrushKind.bkNone]) or (not FBrushAsBitmap)) and
...
begin
  ...
  begin
    ...
    if (Obj.Obj.Style <> nil) and (Obj.Obj.Style.Color = claNull) then
    begin
